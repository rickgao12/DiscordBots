...
module.exports.run = async (bot, message, command, args) => {
	...
	try {
		...
		const URL1 = `https://${region}.api.riotgames.com/lol/summoner/v4/summoners/by-name/${args[0]}?api_key=${process
			.env.LOL_API}`;
		...
		if (summonerData) {
			...
			const embed = new Discord.MessageEmbed()
				.setTitle(name)
				.setColor(0x42e6f5)
				.addField('Queue Type', summonerData.queueType.split('_').join(' '))
				.addField('Player Rank: ', `${summonerData.tier} ${summonerData.rank} ${summonerData.leaguePoints}LP`)
				.addField('Win Ratio', `${winRatio}%`)
